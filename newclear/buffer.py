...
class InstanceCli(Instance):
    def __init__(self, prog_name=None, version='unknown', prog_args=None, invoke_without_command=False):  # pylint:disable=super-init-not-called
        ...
        self.method = None
        self.method_list = {func.replace("_", "-"): func for func in dir(self) if callable(getattr(self, func)) and not func.startswith("_")}
    ...
    def __execute_method(self, *args):
    ...
    def __help_exit(self, method=None):
        if method is None:
            print(f"you need a method: possibilities {self.method_list.keys()}")
        else:
            print(f"unknown method {method} : possibilities {self.method_list.keys()}")
        sys.exit(1)
...
# def main():
#     InstanceCli(prog_name='example', version='0.0.1')  # pylint:disable=no-value-for-parameter
